...
from utils import xavier_init, prob
...
class imputation ():
  ...
  def impute(self, method, num_threads):
    ...
    # Generator
    if method == 'gip':
      G_sample = generate_gip(X, M)
    elif method == 'gain':
      G_sample = generate_gain(X, M)
    else:
      print ("Method is wrong, please choose a correct method")
  
    # Combine with observed data
    Hat_X = X * M + G_sample * (1-M)
    
    # Discriminator
    D_prob = discriminate(Hat_X, H)
    ...
    #saver.save(sess, "gip.ckpt")
    #saver.restore(sess, "gip.ckpt")

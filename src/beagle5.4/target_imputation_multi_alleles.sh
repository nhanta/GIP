...
bcftools query -l ${output_dir}/SNPID.vcf.gz | \
...
bcftools view -S ^${output_dir}/duplicate_sample_IDs.txt \
   --force-samples ${output_dir}/SNPID.vcf.gz -Ou | \
...
bcftools +fill-tags -Ou -- -t AC,AN,AF 
cp ${output_dir}/SNPID.vcf.gz ${output_dir}/noduplicate_variants.vcf.gz
...
sort -V ${output_dir}/ip_exclude.txt \
    > ${output_dir}/ip_exclusion1.txt
sort -V ${output_dir}/ip_nonpanel_exclude.txt \
    > ${output_dir}/ip_exclusion2.txt

# Exclude the variants showing highly discrepant AF
bcftools view -e ID=@${output_dir}/ip_exclusion1.txt  \
    ${output_dir}/noduplicate_variants.vcf.gz \
    -Oz -o ${output_dir}/exclusion1.vcf.gz

# Exclude the variants not present in the reference panel
bcftools view -e ID=@${output_dir}/ip_exclusion2.txt \
    ${output_dir}/exclusion1.vcf.gz \
    -Oz -o ${output_dir}/for_phasing.vcf.gz
...
for CHR in {1..23}; do 
    ...
    beagle -Xmx96g \
        gt=${output_dir}/for_phasing.vcf.gz \
        ref=${ref_panel}/1000GP_AF_chr${CHR}.vcf.gz \
        map=${genetic_map}/beagle_chr${CHR}_b38.map \
        out=${output_dir}/phased_chr${CHR} impute=false gp=true \
...
for CHR in {1..23}; do
    ...
    # Create index file
    tabix -fp vcf ${output_dir}/phased_chr${CHR}.vcf.gz
	
    # Re-calculate allele frequency and 
    # compute Impute2-like INFO score
    bcftools +fill-tags ${output_dir}/phased_chr${CHR}.vcf.gz \
        -Ou -- -t AF | \
    bcftools +impute-info \
        -Oz -o ${output_dir}/imputed_info_chr${CHR}.vcf.gz
done
...
for CHR in {1..23}; do
    if [ -f ${output_dir}/imputed_info_chr${CHR}.vcf.gz ]; then
        echo ${output_dir}/imputed_info_chr${CHR}.vcf.gz >> $inputed_files
        echo ${CHR} >> ${chr_names}
    else
        echo imputed_info_chr${CHR}.vcf.gz 'does not exist.'
    fi
done

# Concat imputed files

bcftools concat -f ${inputed_files} -Oz -o ${output_dir}/combination.vcf
rm ${inputed_files}

# Sort SNPs
bcftools sort ${output_dir}/combination.vcf -Oz -o ${input_dir}/imputed_beagle.vcf

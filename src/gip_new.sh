...
echo "- bcftools query -f '[%GT ]' ${data_path}/${data_name}.vcf | sed -E 's/[|\/]/ /g' | sed -E 's/\./?/g' > ${data_path}/${data_name}.hap" | tee -a "${log}"
bcftools query -f '[%GT ]' "${data_path}"/"${data_name}".vcf | sed -E 's/[|\/]/ /g' | sed -E 's/\./?/g' > "${data_path}"/"${data_name}".hap
